...
async function uniqueNames(c, table) {
...
async function page(classTableId, query, offset, limit) {
  ...
  const where = needle.length === 0 ? "" : `WHERE class_name ILIKE '%' || ${sqlString(needle)} || '%'`;
...
async function handle(msg) {
  try {
    ...
    if (msg.type === "page") {
      ...
      const out = { id: msg.id, type: "page", ...result };
