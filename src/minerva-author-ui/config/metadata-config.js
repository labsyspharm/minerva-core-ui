...
const to_group_channel = (group, channel, color, expanded) => {
  return {
    UUID: nanoid(),
    State: {
      Expanded: expanded,
...
const to_item_registry = () => {
  ...
  const group_channels = source_channels.map((channel, i) => {
    const size = Math.floor(source_channels.length / groups.length);
    const group = groups[Math.floor(i / size)];
    return to_group_channel(group, channel, colors[i % colors.length], true);
  });
  return {
    Name: "Example Story",
    Stories: [
