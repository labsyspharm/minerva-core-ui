...
class ContrastBitmapLayer extends BitmapLayer {
  ...
  updateState({ props, oldProps, changeFlags }) {
    ...
    const changed = [
      ...
      "channel_color",
      "window_corner",
    ].some((k) => props[k] !== oldProps[k]);
    if (changed) {
      // Set the custom uniform
      const { lower_range, upper_range, channel_color, window_corner } = props;
...
function createTileLayer(meta, tile_info) {
  const { id, visible, color, lowerRange, upperRange } = tile_info;
  const { pyramids } = meta;
  ...
  const tileProps = {
    ...
    extent: [
      0,
      0,
      tileSize * (1 + 2 ** maxLevel),
      1000 + tileSize * (1 + 2 ** maxLevel),
    ],
    minZoom: minZoom,
    maxZoom: -1,
    color: [...color.map((v) => v / 255), 1],
    ...
    getTileData: (args) => {
      ...
      const subpath = pyramid[zoom].frameMappings[`${y + 1}-${x + 1}-${id}`];
      if (!subpath) {
        return null;
      }
      return load(`${meta.series}/instances/${subpath}`, ImageLoader, {
        fetch: async (url) => {
