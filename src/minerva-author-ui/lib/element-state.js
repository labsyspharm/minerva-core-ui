...
function defineElement(element, options = {}) {
  const { globalStyleSheet, customSuffix, closure } = this;
  return ((el) => {
    let n = 0;
    let tag = `${element.name.toLowerCase()}-${customSuffix}`;
    while (customElements.get(tag) !== undefined) {
      n += 1;
      tag = [element.name.toLowerCase(), n.toString(32), customSuffix].join(
        "-",
      );
    }
    if (el.define) {
    ...
    class extends element {
      ...
      constructor() {
        ...
        if (this.elementTemplate) {
          ...
          html`${this.elementTemplate}`(addStyles.call(this, globalStyleSheet));
