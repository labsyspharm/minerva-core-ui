...
class Field extends HTMLElement {
  ...
  static elementProperties = new Map([["markdown", { type: Boolean }]]);

  get elementTemplate() {
    const default_text_choice = this.defineElement(TextField, {
      defaults: { property: "" },
    });
    const default_editor_choice = this.defineElement(MDEditor, {
      defaults: { property: "", linking: false },
      attributes: ["linking"],
    });
    const text_choices = {
      "STORY-DIALOG": this.defineElement(TextFieldStory, {
        defaults: { property: "" },
      }),
    };
    const editor_choices = {
      "STORY-DIALOG": this.defineElement(MDEditorStory, {
        defaults: { property: "", linking: false },
        attributes: ["linking"],
      }),
    };
    ...
    const field = () => {
      const { markdown, label, property, dialog, notice } = this.elementState;
      if (markdown) {
