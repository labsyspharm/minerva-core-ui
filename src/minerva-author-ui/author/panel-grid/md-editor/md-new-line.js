...
const split = (props) => {
  const { skip, type, pos, size, tr } = props;
  if (skip) {
    return false;
  }
  const map = (v) => tr.mapping.map(v);
  const start = Math.max(map(pos), map(tr.curSelection.from));
  const end = Math.min(start + size, map(tr.curSelection.to + 1));
  const step = replaceStep(tr.doc, start, end);
  if (step) tr.step(step);
  tr.split(start, 1, [{ type }]);
  return true;
};
...
const toParagraphs = (state) => {
  ...
  let replacements = 0;
  const { hard_break, paragraph: type } = schema.nodes;
  tr.doc.nodesBetween(pos, to, (node, pos) => {
  ...
  tr.setSelection(
    TextSelection.create(state.apply(tr).doc, pos, to + replacements),
  );
  return tr;
};
...
const enterNewLine = (state, dispatch) => {
  ...
  const map = (v) => tr.mapping.map(v);
  const { nodeBefore } = tr.doc.resolve(selection.from);
  ...
  if (nodeBefore?.type == hard_break) {
    ...
    tr.setSelection(
      TextSelection.create(state.apply(tr).doc, pos - 1, pos + size),
    );
    split({
      type,
      tr,
    ...
    tr.deleteSelection().insert(map(selection.from), hard_break.create());
  }
  dispatch(tr);
  return true;
};
