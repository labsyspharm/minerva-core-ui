...
const lift_test = (state, dispatch) => {
  ...
  state.doc.nodesBetween(
    ...
    state.selection.to,
    (node, pos) => {
      const found = node.type == list_item;
      n_found += found;
      if (found) {
        const map_resolve = (v) => tr.doc.resolve(tr.mapping.map(v));
        const from = map_resolve(pos + 1);
        const range = from.blockRange(map_resolve(pos + node.nodeSize - 1));
...
const toggleStrong = (markType) => {
  return toMenuItem(toggleMark(markType), {
...
const toggleHeading = (nodeType, level) => {
  ...
  return toMenuItem(
    (state, dispatch, view) => {
      ...
      const { hard_break, paragraph } = state.schema.nodes;
      state.doc.nodesBetween(
        state.selection.from,
        state.selection.to,
        (node, pos) => {
      ...
      if (found_heading) {
        return wrapInBlock(paragraph)(state, dispatch, view);
      }
      const tr = toParagraphs(state);
      if (dispatch && tr) {
        state = state.apply(tr);
      ...
      return wrapInBlock(nodeType, attrs)(state, dispatch, view);
    },
    {
